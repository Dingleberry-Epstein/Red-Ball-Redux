...
class Cubodeez_The_Almighty_Cube(pygame.sprite.Sprite):
    ...
    def __init__(self, physics_manager, x, y, target_ball=None, size=150):
        ...
        self._jumping = False
        self._grounded = True  # Start grounded by default
    ...
    def setup_collision_handlers(self):
        ...
        boss_type = self._physics.collision_types["boss"]
        ball_type = self._physics.collision_types.get("ball", 1)
        
        self._physics.space.on_collision(boss_type, ball_type, begin=self.on_hit_player)
        
        print(f"Boss collision handler registered: boss type = {boss_type}, ball type = {ball_type}")
        
        # Boss vs Ground collision - for landing detection
        if "ground" in self._physics.collision_types:
            ground_type = self._physics.collision_types["ground"]
            self._physics.space.on_collision(boss_type, ground_type, begin=self.on_hit_ground)
        
        # Boss vs Launcher collision - destroy launchers on contact
        if "launcher" not in self._physics.collision_types:
            self._physics.collision_types["launcher"] = 6
        
        launcher_type = self._physics.collision_types["launcher"]
        self._physics.space.on_collision(boss_type, launcher_type, begin=self.on_hit_launcher)
    ...
    def on_hit_launcher(self, arbiter, space, data):
        ...
        for shape in arbiter.shapes:
            if hasattr(shape, "launcher") and shape.launcher:
                ...
                space.remove(launcher.body, launcher.shape)
        return True

    def on_hit_ground(self, arbiter, space, data):
        """Handle collisions with ground - fixed pymunk API usage"""
        # ALWAYS allow collision when not jumping (normal grounded state)
        if not self._jumping:
            self._grounded = True
            return True
        
        # During jump, only process collision if we're falling down
        if self._body.velocity.y <= 0:  # Still going up or at peak
            return False  # Phase through ground
        
        # We're falling - get contact point (fixed pymunk API usage)
        ...
        # FORCE LANDING after timeout to prevent infinite loops
        if self._jump_time > 3.0:
            print("FORCED LANDING due to timeout")
            self.handle_landing(contact_x, contact_y - self._size/2)
            return True
        
        # Normal landing logic - be more permissive
        dx = contact_x - self._landing_target_x
        dy = contact_y - self._landing_target_y
        distance = math.sqrt(dx*dx + dy*dy)
        
        # Land if reasonably close OR falling fast
        should_land = (
            distance <= self._landing_target_radius * 1.5 or  # 1.5x more forgiving
            self._body.velocity.y > 600 or                    # Falling fast
            self._jump_time > 2.0                            # Been airborne long enough
        )
        
        if should_land:
            print(f"Landing: distance={distance:.1f}, vel_y={self._body.velocity.y:.1f}")
            self.handle_landing(contact_x, contact_y - self._size/2)
            return True
        else:
            print(f"Rejecting landing: distance={distance:.1f}, vel_y={self._body.velocity.y:.1f}")
            return False
    ...
    def reset_position(self):
        ...
        self._fall_time = 0
        self._state = "idle"
        self._next_action_time = pygame.time.get_ticks() / 1000.0 + 1.0
    ...
    def check_ground_below(self):
        """Check for ground below using ray casts"""
        ...
        check_points = [
            (x, y + box_half_size),
            (x - box_half_size * 0.7, y + box_half_size),
            (x + box_half_size * 0.7, y + box_half_size)
        ]
        ...
        for check_x, check_y in check_points:
            if self._body.velocity.y > 0 or not self._grounded:
                ray_distance = 500
                for dist in range(0, ray_distance, 10):
                    test_y = check_y + dist
    ...
    def check_point_collision(self, x, y):
        """Check if a point collides with any ground shape"""
        if self._jumping and not self._landing_collision_enabled:
            return False
    ...
    def check_target_has_ground(self, x, y):
        """Check if the target position has ground underneath it"""
        for dist in range(0, 500, 20):
            check_y = y + dist
    ...
    def handle_landing(self, x, y):
        """Handle landing - completely reset all states"""
        print(f"=== BOSS LANDING SEQUENCE STARTED ===")
        
        # CRITICAL: Reset ALL jump-related states immediately
        self._jumping = False
        self._grounded = True
        ...
        # Force position if needed (prevent clipping)
        self._body.position = (self._body.position.x, y)
        
        # Enable all collisions immediately
        ...
        self.shake_screen(0.6, 30)
        
        # Check for player squish
        if self._target and hasattr(self._target, 'rect'):
            boss_rect = pygame.Rect(self._body.position.x - self._size/2, 
                                self._body.position.y - self._size/2, 
                                self._size, self._size)
            if boss_rect.colliderect(self._target.rect):
                if hasattr(self._target, 'death') and not (hasattr(self._target, 'is_dead') and self._target.is_dead):
                    self._target.death()
                    if self._squish_sound:
                        self._squish_sound.play()
        
        # Start vulnerability immediately
        self._vulnerable = True
        self._vuln_timer = 0
        self._flash_vulnerable = True
        self._damaged_this_cycle = False
        
        # Go directly to recovering state
        ...
        print(f"=== BOSS LANDED - STATE: {self._state}, GROUNDED: {self._grounded}, JUMPING: {self._jumping} ===")

    def disable_all_collisions_except_player(self):
        """Disable collisions with everything except the player"""
        ball_category = self._physics.collision_types.get("ball", 1)
    ...
    def enable_landing_spot_collision(self):
        """Enable collision with ground at the landing spot - more reliable"""
        ground_category = self._physics.collision_types.get("ground", 1)
        ball_category = self._physics.collision_types.get("ball", 1)
        self._shape.filter = pymunk.ShapeFilter(
            categories=0x1,
        ...
        print(f"Boss landing collision enabled - target: ({self._landing_target_x}, {self._landing_target_y})")
    
    def shake_screen(self, duration, intensity):
        """Set screen shake values"""
        self._shake_timer = duration
        self._shake_amount = intensity
    
    def check_player_squish(self):
        """Check if player is being squished by the boss"""
    ...
    def check_if_falling_off_map(self):
        """Check if boss is falling off the map"""
    ...
    def calculate_jump_target(self):
        ...
        # Target directly where the player is now
        self._target_x = player_pos.x
        self._target_y = player_pos.y
        
        # Set landing target to the same position
    ...
    def jump_to_target(self):
        """Execute a jump to the target position with enhanced trajectory calculation"""
        if not self._grounded or self._jumping:
            return False
                
        # Get current position and calculate displacement
        start_x, start_y = self._body.position
        dx = self._target_x - start_x
        dy = self._target_y - start_y
        
        # Calculate and constrain horizontal distance
        distance = abs(dx)
        ...
        # Get gravity magnitude from physics space
        gravity = abs(self._physics.space.gravity.y)
        
        # Calculate optimal jump timing for visible arc
        time_to_target = max(1.8, math.sqrt(2 * distance / (gravity * 0.6)))
        time_to_target = min(3.2, max(1.8, time_to_target))
        
        # Calculate horizontal velocity component
        velocity_x = dx / time_to_target
        
        # Enhanced vertical velocity calculation for dramatic arc
        min_jump_height = max(350, abs(dy) + 250)
        
        # Calculate required upward velocity for the arc
        velocity_y = -math.sqrt(2 * gravity * min_jump_height)
        velocity_y = max(velocity_y, -1300)  # Cap maximum upward velocity
        
        if self._debug:
            print(f"Jump parameters - velocity: ({velocity_x:.1f}, {velocity_y:.1f}), "
                f"time: {time_to_target:.2f}s, height: {min_jump_height}px")
        
        # Apply jump impulse to physics body
        self._body.velocity = (velocity_x, velocity_y)
        
        # Update jump state variables
        self._jumping = True
        self._grounded = False
        self._jump_time = 0
    ...
    def update_visual_effects(self):
        """Update visual effects including screen shake and vulnerability flashing"""
        # Handle screen shake effect
        if self._shake_timer > 0:
            self._shake_timer -= 1/60
            
            # Generate random shake offset
            shake_x = random.randint(-self._shake_amount, self._shake_amount)
            shake_y = random.randint(-self._shake_amount, self._shake_amount)
            
            # Apply shake to visual position only
            self._image = self._original_image.copy()
            self._rect = self._image.get_rect(center=(
                self._body.position.x + shake_x,
                self._body.position.y + shake_y
            ))
        else:
        ...
        # Calculate eye rendering parameters
        eye_size = self._size // 8
        eye_y = self._size // 3
        eye_color = self._eye_color
        
        # Apply vulnerability visual effects
        if self._vulnerable and self._flash_vulnerable:
            eye_color = (255, 255, 255)  # White flash for eyes
            
            # Add semi-transparent white overlay for body flash
        ...
        # Redraw eyes with current color
    ...
    def update(self):
        """Main update - with better state management"""
        if not self._target:
            return
        ...
        # Update jump timer ONLY when actually jumping
        if self._jumping:
            self._jump_time += 1/60
            
            # CRITICAL: Prevent infinite timeout loops
            if self._jump_time > 4.0:  # Maximum 4 seconds airborne
                print("EMERGENCY LANDING - Jump timeout exceeded")
        ...
        # Update marker
        if self._show_marker and not self._marker_locked and self._target:
        ...
        # Eye color updates
        if self._jumping:
            self._eye_color = (255, 0, 0)  # Red when jumping
        else:
    ...
    def _handle_idle_state(self, current_time):
        """Handle boss behavior during idle state"""
        ...
        timeout_override = current_time > self._next_action_time + 3.0
        
        if action_ready or timeout_override:
            self._state = "preparing"
            self._next_action_time = current_time + 1.0
            
            if self._debug:
                print("Cubodeez transitioning to preparation phase")

    def _handle_preparing_state(self, current_time):
        """Handle preparation state - ensure clean jump setup"""
        prep_start_time = self._next_action_time - 1.0
        glow_progress = min(1.0, (current_time - prep_start_time) / 1.0)
        ...
        if current_time > self._next_action_time:
            ...
            # IMPORTANT: Reset jump states before jumping
            self._jumping = False
            self._grounded = True
            self._descent_phase = False
            self._jump_time = 0
    ...
    def _handle_recovering_state(self, current_time):
        ...
        if current_time > self._next_action_time:
            self._state = "idle"
            self._next_action_time = current_time + 1.2  # Slight delay before next attack
            
            if self._debug:
                print("Recovery complete - returning to idle state")

    def draw(self, screen, camera):
        """Render boss and associated visual elements"""
        # Draw target marker if active
        if self._show_marker:
            marker_screen_rect = camera.apply_rect(self._marker_rect)
            screen.blit(self._target_marker, marker_screen_rect)
            
        # Debug visualization for landing target
        if self._jumping and self._debug:
            # Create landing target indicator
            landing_indicator = pygame.Rect(0, 0, 12, 12)
            landing_indicator.center = (self._landing_target_x, self._landing_target_y)
            
            # Transform to screen coordinates
            screen_landing_rect = camera.apply_rect(landing_indicator)
            landing_center = screen_landing_rect.center
            
            # Draw target visualization
            pygame.draw.circle(screen, (0, 255, 0), landing_center, 25, 3)  # Green circle
            pygame.draw.rect(screen, (0, 255, 0), screen_landing_rect)        # Center dot